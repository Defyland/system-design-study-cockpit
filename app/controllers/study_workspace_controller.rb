class StudyWorkspaceController < ApplicationController
  ...
  def guide
    ...
    @documents = @source_ids.any? ? StudyDocument.where(id: @source_ids).ordered : StudyDocument.ordered
    ...
    @selected = @groups[@active_tab].first if params[:section].blank? && @groups[@active_tab].any?
  end
  ...
  def map
    ...
    @selected_document = scope.find_by(id: params[:document_id]) if params[:document_id].present?
  ...
  def configure
    ...
    if @source_ids.any? && @documents.size == @source_ids.size
      cards = StudyCardCatalog.new.cards.select { |card| StudyCardCatalog.in_topic?(card, @topic) && @source_ids.include?(card[:document_id]) }
      ...
      @quiz_count = quiz.cards.count { |card| StudyCardCatalog.in_topic?(card, @topic) && (quiz.source_document_ids(card) & @source_ids).any? && !answered.include?(card[:id]) }
      return
    end
  ...
  def start
    ...
    source_ids = selected_source_ids
    return head :unprocessable_entity if source_ids.empty? || StudyDocument.where(id: source_ids).count != source_ids.size

    case params[:format]
    when "guide" then redirect_to study_guide_path(document_id: source_ids.first, source_ids: source_ids)
    ...
    when "quiz"
      ...
      answered = StudyQuizRound.answered_keys(learner_key)
      keys = catalog.cards.select { |card| StudyCardCatalog.in_topic?(card, topic) && (catalog.source_document_ids(card) & source_ids).any? && !answered.include?(card[:id]) }.map { |card| card[:id] }.take(50)
      return redirect_to study_configure_path(topic: topic, source_ids: source_ids), notice: "Estas fontes não têm perguntas inéditas elegíveis no Arcade." if keys.empty?
    ...
    when "cards"
      ...
      completed = StudyCardRound.completed_keys(learner_key)
      keys = StudyCardCatalog.new.cards.select { |card| StudyCardCatalog.in_topic?(card, topic) && source_ids.include?(card[:document_id]) && (replay == completed.include?(card[:id])) }.map { |card| card[:id] }.take(50)
      return redirect_to study_configure_path(topic: topic, source_ids: source_ids), notice: replay ? "Nenhum card lido nestas fontes." : "Nenhum card inédito nestas fontes. Revisão é opcional." if keys.empty?
