...
class EnglishArcadeCardTest < ActiveSupport::TestCase
  ...
  test "mastery needs two high-quality variants separated by about a week" do
    ...
    second = EnglishArcadeAttempt.create!(attempt_attributes(session, "rephrase", @at + 7.days, 8))

    assert_equal "initial", first.variant_key
    assert_equal "rephrase", second.variant_key
  ...
  def attempt_attributes(session, variant, answered_at, quality_score)
    {
      ...
      target: "dsa",
      card_key: "dsa-1",
      attempt_kind: variant == "initial" ? "initial" : "rephrase",
      variant_key: variant,
      answer_choice: "a",
      correct: true,
